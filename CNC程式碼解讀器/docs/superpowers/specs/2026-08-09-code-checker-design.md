...
- **單元測試（Vitest）**：`codeChecker.test.js` 針對每條規則寫測試：
  ...
  - 用真實 A/B/C/D.NC 樣本跑一次，確認無誤報（重點：WHILE/END1、T0M6、G#100、M01、括號註解這些合法模式不得報錯）
...
2. 在編輯器插入明顯錯誤（刪除 END1／未閉括號／未知 G99）→ 3 秒內錯誤面板更新、錯誤行出現紅點
3. 點錯誤列 → 跳轉到該行
