...
### 觸發關鍵字
當使用者說出以下關鍵字時，**讀取並執行** `.opencode/skills/exam-to-material/SKILL.md`：
- **「把這份考卷變成教材」**
- **放入新的考卷 PDF**
...
當使用 writing-plans 技能寫完實作計畫後：
- **不要詢問**「Subagent-Driven 或 Inline Execution」
- **直接預設使用 Subagent-Driven** 執行（每個任務派新 subagent，任務間審查）
- 僅當使用者明確要求時才改用其他執行方式

---

## 📌 Superpowers Skill 使用規則（重要）

### 預設：只跑 brainstorming
每次動手寫 code 前，**只載入並執行 `brainstorming` skill**，用來釐清需求、探索方案、避免衝動實作。

### 例外：建議走完整流程
當任務符合以下任一條件時，**主動建議**使用者改跑完整 Superpowers 流程（含 TDD、code review、verification 等）：
- 大型重構（影響 5+ 檔案）
- 跨模組或多專案改動
- 高風險功能（金流、權限、資料刪除）
- 使用者明確要求
...
### 禁止
- 不自動載入 TDD、code review、systematic-debugging 等 skill（除非符合上述例外）
- 不因「這很簡單」就跳過 brainstorming
- 不在對話中自動載入 hallmark skill（仍需使用者觸發關鍵字）